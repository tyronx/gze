...
func createTreasures(quantity) {
	...
	while (i++ < attempts && quantity > 0) {
		...
		if (GetMaterial(x,y) == Material("Tunnel")) {
			...
			createTreasureContents(chest, (10*floor_y) / LandscapeHeight());
			quantity--;
		}
...
func createTreasureContents(obj, iterations) {
	while (iterations-- > 0) {
		// no switch-case :(
		var rnd = Random(18);
		if (rnd == 0) obj->CreateContents(SKUL,0,RandomX(1,3));
		if (rnd == 1) obj->CreateContents(BONE,0,RandomX(1,3));
		if (rnd == 2) obj->CreateContents(WOOD,0,RandomX(1,4));
		if (rnd == 3) obj->CreateContents(ARWP);
	...
	if (Random(2)) {
		obj->CastObjects(SKUL, Random(3), 30);
		obj->CastObjects(BONE, Random(6), 30);
	}
}
/*

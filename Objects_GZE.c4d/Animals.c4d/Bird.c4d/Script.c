...
protected func Survive()
{
	...
	if (InLiquid() || GBackLiquid(GetXDir()*2, GetYDir()*2) || GBackLiquid(GetXDir()*3, GetYDir()*3) || FindObject2(Find_InRect(0, -20 + GetXDir() * 2, -20 + GetYDir() * 2, 40, 40), Find_OCF(OCF_OnFire))) {
		SetComDir(COMD_Up);
	}
}
...
{
	if (Stuck() || GetAction() != "Fly") return 0;

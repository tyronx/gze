...
private func RedefinePhysical(szPhys, idTo) {
	...
	var physNew; 
	if (physDefTo) {
		physNew=BoundBy(physDefTo-physDefFrom+physCurr, 0, 100000);
	}
...
protected func Scaling() {
	var szDesiredAction;
	if (GetYDir() > 0) {
		szDesiredAction = "ScaleDown"; 
	} else {
		szDesiredAction = "Scale";
	}
	if (GetAction() != szDesiredAction) {
		SetAction(szDesiredAction);
	}
	return 1;   
}

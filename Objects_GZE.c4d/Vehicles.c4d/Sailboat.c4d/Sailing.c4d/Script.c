...
func VehicleGrabArea() { return [-20, -5 , 40, 28]; }
// Area in which the Vehicle can be held while being transported
func VehicleHoldingArea() { return [-15,0,30,20]; }
...
func CanRelayControl(controltype, pClonk) {
	if (controltype == "ControlDig" || controltype == "ControlLeft" || controltype == "ControlRight") {
		return GetAction() == "Anchoring";
	}
	return 1; 
}
